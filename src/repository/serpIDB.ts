...
export async function initializeSERP(dbName: string, taskName: string): Promise<void> {
  return new Promise((resolve, reject) => {
    const openReq: IDBOpenDBRequest = indexedDB.open(dbName, 1);

    openReq.onupgradeneeded = async () => {
      const db = openReq.result;
      db.createObjectStore('cookie', { keyPath: 'id' }).createIndex('domain', 'domain', { unique: false });
      db.createObjectStore('page', { autoIncrement: true }).createIndex('start_uri', 'start_uri', { unique: false });
      db.createObjectStore('junction', { autoIncrement: true }).createIndex('junction', 'page_id', { unique: false });
      console.log('SERP Table Initialized.');
    };
    ...
    openReq.onsuccess = async () => {
      ...
      const cookieFileUrl = chrome.runtime.getURL(`init/serp/${taskName}/cookie_essential.csv`);
      const serpCookieArgs: string[] = await getLinesFromFile(cookieFileUrl);

      const pageFileUrl = chrome.runtime.getURL(`init/serp/${taskName}/page_essential.csv`);
      const serpPageArgs: string[] = await getLinesFromFile(pageFileUrl);

      const junctionFileUrl = chrome.runtime.getURL(`init/serp/${taskName}/pc_junction_essential.csv`);
      const serpJunctionArgs: string[] = await getLinesFromFile(junctionFileUrl);
...
export async function getAllPageIn(dbName: string): Promise<SerpPageTable[]> {
  return new Promise((resolve, reject) => {
...
export async function getResultRangedIn(dbName: string, page: number): Promise<SerpPageTable[]> {
  return new Promise((resolve, reject) => {
    const openReq = indexedDB.open(dbName, 1);
    openReq.onsuccess = () => {
      const db: IDBDatabase = openReq.result;
